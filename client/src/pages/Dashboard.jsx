...
const items = [
  ...
  { to: '/search-ride', title: 'Search Rides', desc: 'Find rides matching your route and interests', icon: '🔍', tone: 'from-cyan-500 to-sky-400' },
  { to: '/requests', title: 'My Requests', desc: 'View and manage your ride requests', icon: '📝', tone: 'from-pink-500 to-fuchsia-400' },
  { to: '/history', title: 'Ride History', desc: 'Track completed rides and reviews', icon: '📚', tone: 'from-emerald-500 to-lime-400' },
];
    ...
    <section className="glass-card p-10 max-w-6xl mx-auto space-y-8">
      ...
      {analytics && (
        <div className="grid md:grid-cols-4 gap-3">
          <div className="bg-white rounded-2xl p-4 border">Rides: <b>{analytics.totalRides}</b></div>
          <div className="bg-white rounded-2xl p-4 border">Saved: <b>₹{analytics.moneySaved}</b></div>
          <div className="bg-white rounded-2xl p-4 border">Distance: <b>{analytics.distanceTravelled} km</b></div>
          <div className="bg-white rounded-2xl p-4 border">Level: <b>{analytics.level}</b></div>
        </div>
      )}
      ...
      {suggestion && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-2xl p-4">
          ...
          <p>{suggestion.from} → {suggestion.to}</p>
        </div>
      )}
